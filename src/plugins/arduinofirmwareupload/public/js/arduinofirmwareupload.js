	...
	$("#settings H4:contains('Runtime Settings:')").append(
          '<div class="settings-block"> \
            ...
            <div class="control-group"> \
              ...
              <div class="alert alert-error" id="arduinoFirmware-error" data-bind="css: { hide: selectedFileName() === \'\' || isValidFirmwareFile() }"> \
                <button type="button" class="close" data-dismiss="alert">x</button> \
                <strong>Oh snap!</strong> Only <i>.zip</i>, <i>.tar.gz</i> and <i>.ino</i> files are supported! \
              </div> \
              <div class="alert alert-error" id="file-upload-notsupported" data-bind="css: { hide: ! browserDoesSupportFileApi() }"> \
                <button type="button" class="close" data-dismiss="alert">x</button> \
...
\
	      ...
	      <i class="icon-upload icon-white"></i> Upload firmware from SD card to Arduino</button>\
            ...
            <h4>内存操作</h4> \
            <div class="control-group"> \
			<button href="#copy-to-emmc-dialog" class="btn btn-success" type="button" id="emmc-upload" data-toggle="modal"> \
			<i class="icon-download icon-white"></i> 写入到内存</button> \
			</div>\
			</div>');
	$("#settings").after('<div class="modal fade" id="firmware-upload-dialog"> \
          ...
          <div class="modal-body"> \
            <strong>Filename: </strong><span data-bind="text: selectedFileName"></span> \
            <br> \
            <hr> \
            ...
            <div class="progress-step"> \
              ...
              <ul class="unstyled"> \
                <li data-bind="css: { muted: ! unpacking() }">Unpacking <i class="icon-fire" data-bind="visible: unpacking() && ! unpacked() "></i><i class="icon-ok" data-bind="visible: unpacked"></i> \
	<i class="icon-warning-sign" data-bind="visible: unpacking() && ! unpacked() && failed()"></i></li> \
                <li data-bind="css: { muted: ! compiling() }">Compiling <i class="icon-fire" data-bind="visible: compiling() && ! compiled()"></i><i class="icon-ok" data-bind="visible: compiled"></i> \
		<i class="icon-warning-sign" data-bind="visible: compiling() && ! compiled() && failed()"></i></li><li data-bind="css: { muted: ! arduinoUploading() }">Upload to Arduino <i class="icon-fire" data-bind="visible: arduinoUploading() && ! arduinoUploaded()"></i> \
		<i class="icon-ok" data-bind="visible: arduinoUploaded"></i><i class="icon-warning-sign" data-bind="visible: arduinoUploading() && ! arduinoUploaded() && failed()"></i></li>              </ul> \
            </div> \
            <div class="progress-step"> \
              <a href="#" class="btn" id="arduinoFirmware-showdetails" data-bind="css: { disabled: ! inProgress() }">Show details</a> \
              <div class="collapse" data-bind="css: { collapse: !detailsVisible() }"> \
                <textarea id="arduiniFirmware-details" readonly="readonly" data-bind="text: details"></textarea> \
              </div> \
            </div> \
          </div> \
          <div class="modal-footer"> \
            <a href="#" class="btn" data-toggle="modal" data-target="#firmware-upload-dialog" data-bind="css: { disabled: inProgress() }, visible: ! arduinoUploaded()">Cancel</a> \
            <a href="#" class="btn btn-primary" id="arduinoFirmware-startupload" data-bind="visible: ! arduinoUploaded()">Apply new firmware</a> \
            <a href="#" class="btn btn-primary" id="arduinoFirmware-closeupload" data-toggle="modal" data-target="#firmware-upload-dialog" data-bind="visible: arduinoUploaded() || failed()">Close</a> \
          </div> \
        </div>');
	
    $("#settings").after('<div class="modal fade" id="copy-to-emmc-dialog"> \
		<div class="modal-header"> \
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>\
            <h3>写入内存</h3> \
          </div> \
		  <div class="modal-body">\
		 		<textarea id="write-emmc-details" readonly="readonly" data-bind="text: emmc_details" style="width:96%;height:170pt"></textarea> \
		</div>\
		  <div class="modal-footer"> \
			<a href="#" class="btn" data-dismiss="modal" aria-hidden="true" id="copy-to-emmc-close">关闭</a>\
			<a href="#" class="btn btn-primary" id="copy-to-emmc-apply" data-bind="visible: !writeemmc_running()">写入</a>\
          </div> \
        </div>');
            ...
            */
            ...
            $("#arduinoFirmware-startupload").click(function() {
              fileReader.onload = function(evnt) {
                ...
                self.cockpit.socket.emit('arduinofirmware-upload', { 'filename' : selectedFile.name, data : evnt.target.result });                
              }
              if (!selectedFile) {
              ...
              self.cockpit.socket.emit('arduinofirmware-startupload', { 'filename' : selectedFile.name, 'size' : selectedFile.size });
	      selectedFile = null;
            });
            ...
            $('#copy-to-emmc-close').click(function() {
              self.arduinoFirmwareVM.reset();
              //$('#firmware-upload-dialog').modal('hide');
            });
            ...
            $('#copy-to-emmc-apply').click(function() {
             self.arduinoFirmwareVM.writeemmc_running(true);
              self.cockpit.socket.emit('copy-to-emmc',{'a':'b'});
            });
            ...
            this.arduinoFirmwareVM.details.subscribe(function(data) { 
	         ...
	         $('#write-emmc-details').scrollTop($('#write-emmc-details')[0].scrollHeight);
	...
	this.cockpit.socket.on('copy-to-emmc-status', function(data){
	  arduinofirmware.arduinoFirmwareVM.emmcOutput(data);
	});
	this.cockpit.socket.on('write-to-emmc-failed', function(data){
	  arduinofirmware.arduinoFirmwareVM.writeemmc_running(false);
	});
	this.cockpit.socket.on('write-to-emmc-completed', function(data){
	  arduinofirmware.arduinoFirmwareVM.writeemmc_running(false);
	});
    };
...
function ArduinoFirmwareViewModel(){
    ...
    self.emmc_details = ko.observable('点击写入将系统重内存卡写入内存，如果系统内无内存卡请不要尝试写入内存');
    self.writeemmc_running = ko.observable(false);
    ...
    self.reset = function() {
        ...
        self.details('');
        self.emmc_details('点击写入将系统重内存卡写入内存，如果系统内无内存卡请不要尝试写入内存');
        self.writeemmc_running(false);
	self.failed(false);
    };
    ...
    self.emmcOutput = function(data) {
		self.emmc_details(data);
	}
}
