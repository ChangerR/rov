...
var CONFIG = require('./config')
  ...
  ,	CopytoEmmc = require('../' + CONFIG.CopytoEmmc)
...
var OpenROVArduinoFirmwareController = function(eventLoop) {
  ...
  controller.writer = new CopytoEmmc();
  controller.initializeSocket = function(socket) {

    controller.socket = socket;

    if (!path.existsSync(tempDirectory)) fs.mkdirSync(tempDirectory, 0755);
	socket.on('copy-to-emmc',function(data) {
		console.log('going to run copy-to-emmc.sh');
		controller.writer.install();
	});
  ...
  controller.writer.on('write-to-emmc-failed',
	function(data) {
		controller.socket.emit('copy-to-emmc-failed', data);
	});
	
  controller.writer.on('write-to-emmc-completed',
	function(data) {
		controller.socket.emit('copy-to-emmc-completed', data);
	});
	
  controller.writer.on('write-to-emmc-error-output',
	function(data) {
		controller.socket.emit('copy-to-emmc-status', data);
	});
	
  controller.writer.on('write-to-emmc-output',
	function(data) {
		controller.socket.emit('copy-to-emmc-status', data);
	});
